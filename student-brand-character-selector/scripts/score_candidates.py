...
def lookup_market(candidate: str, concept: str, market: Dict[str, Dict[str, float]], competitors: List[Dict[str, str]]) -> Tuple[float, str]:
    ...
    if matches:
        ...
        adjusted = clamp(score - max(0, comp_sim - 0.72) * 0.8)
        return adjusted, f"market keyword matched: {k}; competitor similarity {comp_sim:.2f} {comp_term} {comp_src}".strip()
    baseline = 0.30 if market or competitors else 0.18
    adjusted = clamp(baseline - max(0, comp_sim - 0.72) * 0.8)
    return adjusted, f"no direct market keyword; competitor similarity {comp_sim:.2f} {comp_term} {comp_src}".strip()
...
def score_name(row: Dict[str, str], market: Dict[str, Dict[str, float]], ip_rows: List[Dict[str, str]], competitors: List[Dict[str, str]], evidence_level: str) -> Dict[str, object]:
    ...
    market_score, market_note = lookup_market(cand, concept, market, competitors)
...
def create_pair_rows(name_rows: List[Dict[str, object]], char_rows: List[Dict[str, object]], evidence_level: str) -> List[Dict[str, object]]:
    ...
    for n in names:
        for c in chars:
            ...
            status = "conditional_pass" if score >= 80 and evidence_level != "idea_only" else "recommend" if score >= 72 else "needs_research"
            if n.get("decision") == "needs_research" or c.get("decision") == "needs_research":
...
def write_research_checklist(path: Path, candidates: List[Dict[str, object]]) -> None:
    ...
    for c in candidates:
        ...
        for task, source in [
            ...
            tasks.append({"candidate": name, "task": task, "source": source, "status": "todo", "evidence_url_or_note": "", "result": ""})
...
def evidence_level(args: argparse.Namespace) -> str:
    ...
    if args.market_csv or args.ip_csv or args.competitor_csv:
...
def main() -> None:
    ...
    name_scores = sorted([score_name(r, market, ip_rows, competitors, level) for r in names], key=lambda r: float(r["total_score"]), reverse=True)
    char_scores = sorted([score_character(r, ip_rows, level) for r in chars], key=lambda r: float(r["total_score"]), reverse=True)
    pairs = create_pair_rows(name_scores, char_scores, level)
    ...
    summary = {"evidence_level": level, "name_count": len(name_scores), "character_count": len(char_scores), "pair_count": len(pairs), "top_pair": pairs[0] if pairs else None}
